...
# union de listas de especies (Giraldo et al. 2018) y sin贸nimos (GBIF)
...
# busqueda de registros geogr谩ficos de cada especie (hasta 5000 registros)
# con coordenadas geogr谩ficas en GBIF
...
# Actualizaci贸n del nombre de las especies seg煤n GBIF Backbone Taxonomy
...
plot(col_dem, main = "Modelo de Elevaci贸n Digital - Colombia")
...
## limpieza de puntos at铆picos basados en literatura (Montoya et al. 2021)
...
sp <- sp[!(sp$scientificName=="Dichotomius agenor" & # Remove D. agenor departments based on Montoya et al. 2021
             sp$stateProvince %in% c("Boyac谩",  "Meta","Casanare", "Vichada", "Boyac脙隆")), ] # 1824 records
sp <- sp[!(sp$decimalLongitude == -76.09989 & sp$decimalLatitude == 8.039917), ] # remove Urab谩 recrord, 1823 records
...
# Inicializar sesi贸n (activar Google Drive para exportaciones)
ee_Initialize(
...
# Elevaci贸n  ALOS PALSAR 30m de GEE
...
# Asegurar que elev_sf est谩 en el mismo CRS
...
# Filtrar puntos por distancia m铆nima de 1 km
...
if (!"elev_ALOS" %in% names(sp_df)) {
  stop("elev_ALOS se perdi贸 en la conversi贸n a data.frame. Revisa antes de filtrar.")
}
...
###
# extracci贸n del "area de movilidad (M)" basado en regiones biogeogr谩ficas (WWWF ecoregions)
###
